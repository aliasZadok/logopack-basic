...
const CustomDropdown: React.FC<CustomDropdownProps> = ({ onSizeChange }) => {
    const [isOpen, setIsOpen] = useState(false);
    const [selectedSize, setSelectedSize] = useState<string | number>('Select size');
    const [customSize, setCustomSize] = useState<number | ''>('');
    const dropdownRef = useRef<HTMLDivElement>(null);

    const predefinedSizes = [300, 600, 900, 1200];
    ...
    return (
        <div className={styles.dropdownContainer} ref={dropdownRef}>
            <button className={styles.dropdownTrigger} onClick={handleToggleDropdown}>
                {typeof selectedSize === 'number' ? selectedSize : 'Select Size'}  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="currentColor" className="bi bi-chevron-down" viewBox="0 0 16 16"> <path fill-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708"/> </svg>
            </button>
            {isOpen && (
